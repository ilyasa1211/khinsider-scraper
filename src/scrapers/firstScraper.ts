...
export default class FirstScrapper {
  public static async getTitle(page: Page) {
    ...
    const rawTitle = await titleLocator.innerText(); // 'Plants vs. Zombies Original Soundtrackinfo'

    return rawTitle.slice(0, rawTitle.lastIndexOf("info")); // 'Plants vs. Zombies Original Soundtrack'
  }

  public static async getDownloadPageUrl(page: Page, targetUrlOrigin: string): Promise<FirstScrapeResult[]> {
